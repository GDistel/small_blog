...
import { PagedResponse } from '../interfaces/pagination.interface';
...
export class Pagination {
    ...
    private processLimitString(limit: number): number {
        if (limit === undefined || limit === null || limit < 0 || typeof limit !== 'number') {
            return 3;
        };
        const validLimit = limit > this.maxLimit ? this.maxLimit : limit;
        return Math.floor(validLimit) ?? this.maxLimit;
    }
    ...
    public paginateItems<T>(data: Array<T>, total: number): PagedResponse<T> {
        return {
            data,
            total,
            page: this.page,
